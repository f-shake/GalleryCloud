...
interface Stats { totalPhotos: number; totalUsers: number; totalSizeGb: number; photosWithGps: number; formatDistribution: { format: string; count: number }[] }
const stats = ref<Stats | null>(null)
const loading = ref(true)

onMounted(async () => {
  try { const r = await client.get('/admin/stats'); stats.value = r.data }
  catch { /* */ }
  ...
  <div style="padding:16px">
    <el-skeleton :loading="loading" animated :count="1">
      <div v-if="stats" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px;margin-bottom:24px">
        <el-statistic title="照片总数" :value="stats.totalPhotos" />
        <el-statistic title="用户数" :value="stats.totalUsers" />
