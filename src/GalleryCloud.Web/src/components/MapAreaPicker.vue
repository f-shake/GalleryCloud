...
async function ensureMapInitialized() {
  ...
  initPromise = (async () => {
    ...
    if (props.south != null && props.north != null && props.west != null && props.east != null) {
      ...
      sketchLayer.add(new Graphic({ geometry: polygon }))
      hasSelection.value = true
...
<template>
  ...
  >
    <div class="map-area-body">
      <div ref="mapContainer" class="map-area-map" :class="{ 'map-area-map--loading': loading }">
        ...
        <div v-if="!loading" class="map-area-toolbar">
          ...
          <el-button size="small" :type="hasSelection && !drawing ? 'primary' : 'default'" :disabled="drawing" @click="startDraw">
    ...
    <div class="map-area-footer">
      <div v-if="hasSelection" class="map-area-coords">
        <span class="map-area-coord-label">已选区域</span>
        <span v-if="readExtent()" class="map-area-coord-val">{{ readExtent()!.areaKm2 }} km²</span>
      </div>
